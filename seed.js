...
async function main() {
  ...
  if (existingByUsername) {
    ...
    console.log("Admin account updated (password synced from .env) ✅");
    return;
  }
  ...
  if (existingAdmin) {
    console.log(
      `SUPER_ADMIN already exists with username "${existingAdmin.username}". ` +
        `Set ADMIN_USERNAME to that value or delete the user, then re-run seed.`,
    );
    return;
  }
